...
async function handleWeaponsCommand(sock, from, sender, text, isGroup, participants, senderName, overrider = ["VITIN", "JESSE"]) {
  ...
  if (command === `${prefix}armas`) {
    if (!overrider.includes(senderName)) {
      await sock.sendMessage(from, {
        text: "❌ Apenas overrider podem usar este comando.",
      })
      return true
    }
    ...
    const armasMenu = `
╭━━━〔 ⚔️ MENU: ARMAS 〕━━━╮
│ Comandos exclusivos para overrider:
│
│ ${prefix}hiroshima
...
│    por 3 minutos (exceto overrider)
...
╰━━━━━━━━━━━━━━━━━━━━╯
  ...
  if (command === `${prefix}hiroshima`) {
    ...
    const targets = groupMembers.filter(jid => !overrider.includes(jid.split("@")))
    ...
    const randomName = randomTarget.split("@")
    ...
    const mentionsList = targets.map(jid => `@${jid.split("@")}`).join(", ")
  ...
  if (command === `${prefix}nagasaki`) {
    ...
    for (const jid of targets) {
      await applyPunishment(sock, from, jid, "10", {
        severityMultiplier: 1,
        origin: "admin",
      })
    }
