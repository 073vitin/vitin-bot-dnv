...
test("utility router handles !teste and expands mention identity variants", async () => {
  ...
  assert.match(outputText, /RAW_MENTIONED_JID/)
  assert.match(outputText, /REGISTRATION_CANONICAL/)
  assert.match(outputText, /@5511999999999/)
  assert.ok(!/@s\.whatsapp\.net/i.test(outputText))
  assert.ok(!/@lid/i.test(outputText))
  assert.ok(Array.isArray(sent[0].payload?.mentions))
